...
location_before = ppn % temp_total_plane;//step1: get the location in different plane
//step2, get detailed location by location_before value.
location->channel = location_before % ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) % (ssd->parameter->channel_number);// %24
location->chip = location_before % ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) / (ssd->parameter->channel_number);// /24

location->die = location_before / ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) / (ssd->parameter->plane_die);//   /24/2 

location->plane = location_before / ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) % (ssd->parameter->plane_die); // /24%2
location->block = (ppn / temp_total_plane) / (ssd->parameter->page_block);
location->page = (ppn / temp_total_plane) % (ssd->parameter->page_block);
...
//lxc for compression
//here is for the consecutive ppn to the consecutive lpn value.
unsigned int  get_ppn_in_different_case(struct ssd_info* ssd, unsigned int channel, unsigned int chip,unsigned int die,unsigned int plane,unsigned int block,unsigned int page)
{

    unsigned int channel_num = 0, chip_num = 0, die_num = 0, plane_num = 0;
    unsigned int i, plane_die, plane_chip, plane_channel, temp,  temp1, ppn_begin, ppn;
    ...
    plane_chip = die_num * plane_die;
    plane_channel = chip_num * plane_chip;

    temp = channel_num * chip_num * die_num * plane_num;
    //    int* position_change_table = (int*)malloc(size(int)* temp);
    //    for(i = 0; i < temp; i ++){
    //*(position_change_table + i) = 
    //    }
    if(temp != 96){
        ssdsim_msg("should change position_change_table here");
        while(1){

        }
    }
    int position_change_table[96] ={\
        0,24,48,72,   6,30,54,78,  12,36,60,84,  18,42,66,90,\
            1,25,49,73,   7,31,55,79,  13,37,61,85,  19,43,67,91,\
            2,26,50,74,   8,32,56,80,  14,38,62,86,  20,44,68,92,\
            3,27,51,75,   9,33,57,81,  15,39,63,87,  21,45,69,93,\
            4,28,52,76,  10,34,58,82,  16,40,64,88,  22,46,70,94,\
            5,29,53,77,  11,35,59,83,  17,41,65,89,  23,47,71,95\
    };

    temp1 = channel*plane_channel + chip*plane_chip + die*plane_die + plane;
    ppn_begin = position_change_table[temp1];
    ppn = ppn_begin + ((ssd->parameter->page_block * block) + page) * temp;//this is for firstly getting the begin and other's for the fixed offset 

    //        ssdsim_msg("channel %d, chip %d,  die %d, plane %d, block %d, page %d------> produce ppn %d", channel, chip, die, plane, block, page, ppn);
    //        printf("channel %d, chip %d,  die %d, plane %d, block %d, page %d------> produce ppn %d\n", channel, chip, die, plane, block, page, ppn);

    return ppn;
}//end of get_ppn_in_different_case
        ...
        case 2:
            {
                ...
                ppn = get_ppn_in_different_case(ssd, channel, chip, die, plane, block, page);
                break;
            }
//        case 2:
//            {break;}
...
//lxc for compression 
    ...
    flush_mappingtable_into_files(ssd, "map_pre_process.txt");
...
unsigned int get_ppn_for_pre_process(struct ssd_info *ssd, unsigned int lsn) {
    ...
    } else if (ssd->parameter->allocation_scheme == 1)                       /*表示静态分配，同时也有0,1,2,3,4,5这6中不同静态分配方式*/
    {
        switch (ssd->parameter->static_allocation) {
            ...
            case 2: {
                        ...
                        location_before = lpn % temp_total_plane;//step1: get the location in different plane
                        channel = location_before % ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) % (ssd->parameter->channel_number);// %24
                        chip = location_before % ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) / (ssd->parameter->channel_number);// /24

                        die = location_before / ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) / (ssd->parameter->plane_die);//   /24/2 

                        plane = location_before / ( (ssd->parameter->channel_number)*(ssd->parameter->chip_channel[0]) ) % (ssd->parameter->plane_die); // /24%2
    ...
    if (find_active_block(ssd, channel, chip, die, plane) == FAILURE) {
        ...
        ssdsim_msg("wrong in get_ppn_for_pre_process");
        while(1){
        }
        return 0;
    }
    ...
    return ppn;
}//end of get_ppn_for_pre_process
