...
//lxc for  compression 
typedef struct buffer_info_for_DFTL {
    unsigned long read_hit;                      /*这里的hit都表示lpn的命中次数或是没命中的次数*/
    unsigned long read_miss_hit;
    unsigned long write_hit;
    unsigned long write_miss_hit;
    struct buffer_group_for_DFTL *buffer_head;            /*as LRU head which is most recently used*/
    struct buffer_group_for_DFTL *buffer_tail;            /*as LRU tail which is least recently used*/

    TREE_NODE *pTreeHeader;                     /*for search target lsn is LRU table*/

    unsigned int max_buffer_size; // can be maximum 4GB size here.
