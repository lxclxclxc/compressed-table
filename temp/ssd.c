...
void make_real_evict_pages_for_compression_part(struct ssd_info* ssd, struct request* request, int should_evict_size)
{
    ...
    while(should_evict_size > 0){
        ...
        temp_tail = temp_tail->LRU_link_next;
    }
...
int compression_compare_DFTL_buffer_remainder_space_with_compression_area_coming(struct ssd_info* ssd, struct request* request)
{
    ...
    int compression_to_uncompression_size, uncompression_existing_void_size, uncompression_incoming_page_number, prediction_page_size, adding_size_in_compression, compression_part_existing_void_size, evicting_pagenumber_from_uncompression_part , evicting_size_from_uncompression_part ;
...
void COMPRESSION_read_value_request_DFTL_parameters(struct ssd_info* ssd, struct request* req)
{
    ...
    unsigned int need_distb_flag = 0, lsn, lpn,last_lpn, first_lpn, index;
    lsn = req->lsn;
	...
	for(j = 0; j < i; j ++){
		if(temp_arra1[j] != -1){
			...
			for(k = 0; k < NUM_ENTRIES_PER_DIR; k ++){
				*(translation_page_data_before_compression + (NUM_ENTRIES_PER_DIR * j + k)) = ssd->dram->map->map_entry[(NUM_ENTRIES_PER_DIR * j + k)].pn;
			}
...
void get_no_repeat_pages_number_from_lpns(struct ssd_info* ssd, struct request* req,struct _bi_list*  temp_bilist, int* input)
{
    ...
    if(temp_bilist->len > 1){
        for(j = 1; j < i; j++){
            int temp_value = temp_arra[j] /ssd->parameter->page_capacity;
            int flag = 0;

            for(k = 0; k < j; k++){
                if(temp_value == temp_arra[k]/ssd->parameter->page_capacity){
                    flag = 1;
                    break;
                }
            }
            if(flag == 1){
                ...
                input[j] =  temp_arra[j] /ssd->parameter->page_capacity;
            }
        }
    }//end of if
...
int move_pages_from_compression_to_uncompression(struct ssd_info* ssd, struct request* request)
{
    ...
    for(i = 0; i < temp_bilist->len; i ++){
        ...
        //step2, delete the tail nodes in compression part.
        ...
        ssd->dram->mt->buffer_DFTL->buffer_sector_count -= 1 ;// once only one total translation page here.
        ssd->dram->mt->uncompression_now_cached_dir_slots_sizein_byte -=ssd->dram->mt->compression_size[temp_binode->lpn_and_state[0]];
...
if( (req->missing_in_data_buffer_lpn_data != NULL) && ( req->missing_in_DFTL_buffer_lpn_data != NULL) && ( req->should_evict_lpn_data !=NULL) ){
	...
	if( (req->total_wait_fetch_mappingtable == 1)&&((req->total_wait_evict == 1))&&((req->create_or_notyet_evict_subrequests == 1))&&(req->create_or_notyet_fetch_subrequests == 0) ){
		...
		if(req->if_need_movement_between_com_and_uncom_flag_and_page_numbers != 0){
			...
			finished_movement_between_com_and_uncom_flag  = 1;
		}
	}//end of step2
...
//			}
		...
		if((finished_movement_between_com_and_uncom_flag  == 0) && (req->if_need_movement_between_com_and_uncom_flag_and_page_numbers !=0)){
			move_between_compression_and_uncompression(ssd, req);
			finished_movement_between_com_and_uncom_flag  = 1;
	...
	if( (req->total_wait_fetch_mappingtable == 0)&&((req->total_wait_evict == 0))&&((req->create_or_notyet_evict_subrequests == 0))&&(req->create_or_notyet_fetch_subrequests == 0) ){

		if(finished_movement_between_com_and_uncom_flag  == 1){
...
int append_on_request_compression_to_uncompression_lpn_data(struct request* req, int lpn, int state)
{
    ...
    lpush(req->compression_to_uncompression, lpn, state);
...
#ifdef COMPRESSION_MAPPING 
    fprintf(ssd->outputfile, "compression part buffer_DFTL readrequest hits: %13d\n", ssd->dram->mt->buffer_DFTL->read_hit);
    fprintf(ssd->outputfile, "compression part buffer_DFTL writerequest hits: %13d\n", ssd->dram->mt->buffer_DFTL->write_hit);

    fprintf(ssd->outputfile, "total buffer_DFTL readrequest miss: %13d\n", ssd->dram->mt->buffer_DFTL->read_miss_hit);
    fprintf(ssd->outputfile, "total buffer_DFTL writerequest miss: %13d\n", ssd->dram->mt->buffer_DFTL->write_miss_hit);

    fprintf(ssd->outputfile, "only uncompression part,buffer_DFTL readrequest hits: %13d\n", ssd->dram->mt->buffer_DFTL_UNCOM->read_hit);
    fprintf(ssd->outputfile, "only uncompression part,buffer_DFTL writerequest hits: %13d\n", ssd->dram->mt->buffer_DFTL_UNCOM->write_hit);

#else


    fprintf(ssd->outputfile, "buffer_DFTL readrequest hits: %13d\n", ssd->dram->mt->buffer_DFTL->read_hit);
    fprintf(ssd->outputfile, "buffer_DFTL readrequest miss: %13d\n", ssd->dram->mt->buffer_DFTL->read_miss_hit);
    fprintf(ssd->outputfile, "buffer_DFTL writerequest hits: %13d\n", ssd->dram->mt->buffer_DFTL->write_hit);
    fprintf(ssd->outputfile, "buffer_DFTL writerequest miss: %13d\n", ssd->dram->mt->buffer_DFTL->write_miss_hit);

#endif// end of COMPRESSION_MAPPING  definition
#endif
